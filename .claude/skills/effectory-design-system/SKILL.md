...
- **Template selection dialog — "Choose a survey template"** → `reference-prototypes/create-survey-dialog.md` + `create-survey-dialog.html`
  **Dit is de dialog, niet een pagina.** De All surveys-pagina mét werkende **Create survey**-knop, die een `dialog-l` opent: header ("Choose a survey template" + "Start from scratch" naast de zoekbalk), een zoekbalk die **live filtert op naam én omschrijving**, een 3-koloms scrollend grid van template-kaarten (64px illustratie, naam, "Available in N projects", 3-regelige omschrijving, **Use template** + **Preview** die de kaartbreedte delen; op hover wordt Use template primary) en een **No results found**-state met illustratie. **Preview** opent een tweede dialog met de template-details (meta "N questions · N minutes", "Why is it valuable?", **Show more** als Link button, een **Questionnaire**-preview en een zwevende footer met een 326px **Button Big** "Use template"). **Use template** én **Start from scratch** openen dezelfde **"Let's get started"**-dialog (`dialog-s`): survey-naam (3–35 tekens) en — **alleen wanneer de flow vanaf All surveys start** — een select "What project does it belong to?"; vanuit een project vervalt dat veld (`?context=project`). Daarna navigeert het naar de CYOS questionnaire-pagina hieronder.
  *Trigger op o.a.:* template selection dialog · template picker · templatekiezer · "kies een template" · survey template kiezen · choose a survey template · create survey dialog/modal/popup · nieuwe survey aanmaken · survey aanmaken vanaf een template · welke templates zijn er · template overzicht in de dialog · start from scratch · "Let's get started"-dialog · survey een naam geven · project kiezen bij een nieuwe survey.

- **CYOS template questionnaire page — Questions-stap** → `reference-prototypes/survey-settings-questionnaire.md` + `survey-settings-questionnaire.html`
  **Dit is de volledige pagina, niet een dialog.** Het scherm waar een nieuwe survey op landt ná het aanmaken. **Geen hoofdnavigatie**: de flow vervangt de app-shell door een eigen header (Draft-tag, survey-naam, Edit name, Help articles, taal), een **stepper** van 4 stappen (Questions · Participants · Survey period · Layout & emails — 28px cirkel met nummer of check die 4px over de 44px icoontegel valt; done = positive vulling + witte check, actief = brand-selected + brand-secondary), een 1000px contentkolom met de **template-kaart** ("You are not able to change or edit questions in this template") en de **vragenlijst per sectie** (sectienaam + count-pill, per rij de vraag, een scale-chip per vraagtype en een "Standard"-tag), plus een vaste **footer-balk** (Save & Close · Next Step · Review & Plan). **Save & Close** keert terug naar het overzicht waar de flow begon. Gevoed via `?template=…&name=…&context=…`.
  *Trigger op o.a.:* CYOS · create your own survey · CYOS questionnaire page · questions-stap · questions step · vragenlijst van een survey · vragenoverzicht · questionnaire page/overview · survey settings · survey-instellingen · de stappen na het aanmaken van een survey · survey stepper · welke vragen zitten in deze template · template-vragen bekijken · survey configureren/opzetten.

> **Dialog of pagina? — kies zo.** Beide schermen gaan over templates en tonen allebei een vragenlijst, dus scheid ze op **waar de gebruiker is**:
> - Gaat het over **kiezen** — een template uitzoeken, zoeken, vergelijken, previewen, een survey starten → **Template selection dialog**.
> - Gaat het over een survey die **al bestaat** — de vragen erin bekijken, de stappen doorlopen, instellingen → **CYOS template questionnaire page**.
> - Twijfel je? "Dialog/modal/popup" en "aanmaken/kiezen" wijzen naar de eerste; "pagina/scherm", "stap", "stepper" en "deze survey" naar de tweede. Vraagt iemand om de héle flow, bouw dan beide — ze zijn aan elkaar gekoppeld.
...
11. **Publiceer** — eigen repo + eigen Cloudflare Worker, en meld de deelbare URL. Zie regel 16
