...
Tokens komen uit het design system, en **dat is de bron** — niet Figma en niet de dev-styleguide. Nooit handmatig bewerken: lees en gebruik ze.
...
**Dezelfde tag, andere kleur.** De "Standard"-tag in een vragenlijst is exact dit component met een subtiele brand-vulling (`--bg-brand-subtle` + `--content-accent-turquoise`) — geen eigen component, geen ander gewicht. Wijkt een tag in een scherm af in gewicht of grootte, dan is dat een fout in dat scherm, niet een variant.
