...
- **`tokens.css` is auto-gegenereerd** via `build-tokens.py` vanuit een Figma variables-export — bewerk het **nooit** met de hand. Wijzig de tokens in Figma, exporteer de variables, en draai dan `./build-tokens.py <pad-naar-export>` (of zet de export als `figma-variables.json` in de repo-root en draai `./build-tokens.py` zonder argument). `--check` zegt of `tokens.css` en `colors-tokens.js` nog kloppen bij de export, zonder te schrijven. Het pad stond tot 19-08-2026 hardcoded naar één Downloads-map, en dat bestand was inmiddels weg — dus niemand kon tokens meer regenereren.
