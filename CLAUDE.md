...
- **`tokens.css` wordt onderhouden, niet gegenereerd.** Figma is **niet** de bron voor tokens: op 29-07-2026 is `tokens.css` met de hand gelijkgetrokken met de live dev-styleguide (commit `888c966`, 164/164 op het lichte thema) omdat de Figma-waardes fout waren. `build-tokens.py` **vergelijkt** daarom alleen een Figma-export met wat er gecommit staat en schrijft pas met `--write`. Een verschil betekent normaal gesproken dat Figma achterloopt, niet dat `tokens.css` fout is. Wijzigt een tokenwaarde, pas dan `tokens.css` aan tegen de styleguide en leg in de commit vast waarom.
