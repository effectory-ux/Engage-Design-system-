...
- **Het design system is de bron voor tokens, en `tokens.css` wordt onderhouden, niet gegenereerd.** Niet Figma, niet de dev-styleguide: wat hier staat, geldt. Dat die drie gelijk blijven is mensenwerk — er is geen pipeline die het doet, en dat is een bewuste keuze sinds de Figma-waardes op 29-07-2026 fout bleken (commit `888c966`, `tokens.css` toen met de hand gelijkgetrokken, 164/164 op het lichte thema). `build-tokens.py` **vergelijkt** daarom alleen een Figma-export met wat er staat en schrijft pas met `--write`; een verschil is informatie, geen opdracht. Wijzig je een waarde, doe dat hier en leg in de commit vast waarom — en werk Figma en de styleguide zelf bij.
- **`SKILL.md` schrijf je hier, nooit in `effectory-design-documentation`.** Die repo heeft een kopie van de skill, maar die is stroomafwaarts: ververs hem vanuit deze repo, nooit andersom. Doe je het toch omgekeerd, dan bouwt `release-skill.sh` uit `skill-source/` en levert het team een oudere versie — dat ging op 19-08-2026 net goed omdat het toevallig opviel.
- **Controleer beide skill-kopieën** met `./check-skill-consistency.py`: dezelfde regels, dezelfde referentieschermen, dezelfde `reference-prototypes/`-mappen, en geen verwijzing naar een regel die niet bestaat. Draait mee in `./sync-skill.sh`.
- **Alleen font-sizes van de schaal: 12 / 14 / 16 / 20 / 26 / 32 / 42** (`foundation.css`, `.text-l1`…`.text-l6` + `.text-small/-medium/-large`). **15px mag nooit** — die kroop op 28-08-2026 twee keer binnen via gekopieerde prototype-CSS, en naast 14px zie je het verschil niet in een diff maar wel op het scherm. `./tools/check-font-scale.py` faalt erop en draait mee in CI; `docs/styles.css` heeft een eigen halve-stapsysteem (11,5 / 12,5 / 13,5) dat alleen gerapporteerd wordt, maar 15px faalt daar net zo hard. Heb je echt een nieuwe maat nodig, zet die dan eerst als text-class in `foundation.css`.
- **Nooit een native browser-control waar het design system een component voor heeft.** Een `<input type="date">` rendert de widget van de browser: eigen typografie, eigen kleuren, geen tokens, en per OS anders. Er komt geen foutmelding van, dus het valt alleen op in een screenshot — zo stond er op 28-08-2026 een native datumveld in de Focus View-dialog terwijl `dpMonthHTML()` al bestond. `./tools/check-native-controls.py` faalt op `date`/`time`/`month`/`week`/`range`/`file`-inputs en op `<progress>`/`<meter>`, en draait mee in CI. Een bewuste uitzondering markeer je met `check-native-controls: allow` op die regel.
