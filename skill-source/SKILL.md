...
- **`"update": "auto"`** *(standaard)* — haal bij de start van een bouwsessie de nieuwste versie op
  (`./ds-update.sh`) en meld in één zin wat er veranderd is. Zo werk je altijd met het actuele system.
...
4. De gebruiker kan het ook gewoon in de prompt zeggen ("bouw dit maar pin het design system") —
   dat wint van wat er in het bestand staat, en je legt het daarna vast.
5. Draai `./ds-update.sh --check` als je alleen wilt weten of het project achterloopt.

> Op termijn komt deze schakelaar in de prototype-toolbar te staan; tot die tijd is `ds-pin.json`
> plus de prompt de plek waar het geregeld wordt.
