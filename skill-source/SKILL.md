...
- **Create survey — "Choose a survey template" dialog (Coordinator)** → `reference-prototypes/create-survey-dialog.md` + `create-survey-dialog.html`
  De All surveys-pagina mét werkende **Create survey**-knop: een `dialog-l` met header ("Choose a survey template" + "Start from scratch" naast de zoekbalk), een zoekbalk die **live filtert op naam én omschrijving**, een 3-koloms scrollend grid van template-kaarten (64px illustratie, naam, "Available in N projects", 3-regelige omschrijving, **Use template** + **Preview** die de kaartbreedte delen; op hover wordt Use template primary) en een **No results found**-state met illustratie. **Preview** opent een tweede dialog met de template-details (meta "N questions · N minutes", "Why is it valuable?", **Show more** als Link button → "When to use it?" / "What will you get?", een **Questionnaire**-sectie met de vragen gegroepeerd per sectie — count-pill, scale-chip per vraagtype en een "Standard"-tag per rij — en een **zwevende footer** met een 326px **Button Big** "Use template"). **Use template** én **Start from scratch** openen dezelfde **"Let's get started"**-dialog (`dialog-s`): survey-naam (3–35 tekens) en — **alleen wanneer de flow vanaf All surveys start** — een select "What project does it belong to?" met alle projecten van het account; vanuit een project vervalt dat veld (`?context=project`). Vanaf een template gaat het daarna naar `survey-settings-questionnaire.html`. Vragen komen uit de IP-veilige bibliotheek van de `realistic-content` skill. Voor "de create survey-dialog", "template picker", "kies een survey-template", "nieuwe survey aanmaken" of het survey-template-keuzescherm — in welke taal of bewoording dan ook.

- **Survey settings — Questions-stap (CYOS)** → `reference-prototypes/survey-settings-questionnaire.md` + `survey-settings-questionnaire.html`
  Het scherm waar een nieuwe survey op landt na het aanmaken. **Geen hoofdnavigatie**: de flow vervangt de app-shell door een eigen header (Draft-tag, survey-naam, Edit name, Help articles, taal), een **stepper** van 4 stappen (Questions · Participants · Survey period · Layout & emails — 28px cirkel met nummer of check die 4px over de 44px icoontegel valt; done = positive vulling + witte check, actief = brand-selected + brand-secondary), een 1000px contentkolom met de **template-kaart** ("You are not able to change or edit questions in this template") en de **vragenlijst per sectie**, plus een vaste **footer-balk** (Save & Close · Next Step · Review & Plan). **Save & Close** keert terug naar het overzicht waar de flow begon (All surveys, of Projects vanuit een project). Wordt gevoed via `?template=…&name=…&context=…`. Voor "survey settings", "de questions-stap", "het CYOS-scherm", "de vragenlijst van een survey" of het scherm ná het aanmaken van een survey.
