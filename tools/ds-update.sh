...
#
#   tools/ds-update.sh              → latest on main
#   tools/ds-update.sh d7ac1a5      → that exact commit
#   tools/ds-update.sh --check      → report only, change nothing
#
...
if [ "$CHECK" = "1" ]; then
  [ "$CURRENT" = "$LATEST" ] && echo "✓ up to date" || echo "→ an update is available: tools/ds-update.sh"
  exit 0
fi
